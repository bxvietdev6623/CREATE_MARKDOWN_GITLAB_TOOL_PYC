...
#     Each sub_keyword will generate one file:
#     app - url - (main + 3 random keywords) - suffix
#     """
#     try:
#         data = request.get_json()
#         if not data:
...
#         if not isinstance(sub_keywords, list) or not all(isinstance(k, str) for k in sub_keywords):
#             return jsonify({"error": "Invalid or missing 'sub_keywords' list"}), 400
#         if len(sub_keywords) < 4:
#             return jsonify({"error": "Need at least 4 sub_keywords"}), 400

#         # Unique & cleaned sub keywords
#         filtered_unique = list(dict.fromkeys([kw.strip() for kw in sub_keywords if kw.strip()]))
...
#             all_articles = []
#             stt_counter = 1
#             total_count = len(filtered_unique)
#             stt_width = len(str(total_count)) if total_count > 0 else 1

#             # Tạo bài viết cho từng keyword phụ
#             for sk in filtered_unique:
#                 app_fixed = random.choice(FIXED_APPS)
#                 url_fixed = random.choice(FIXED_URLS)
#                 subdomain = ''.join(random.choices('abcdefghijklmnopqrstuvwxyz0123456789', k=3))
#                 domain_link = f"https://{subdomain}.fdswijfrewirk.top/"

#                 # chọn thêm 3 keyword random (không trùng với chính nó)
#                 remaining = [k for k in filtered_unique if k != sk]
#                 if len(remaining) >= 3:
#                     random_subs = random.sample(remaining, 3)
#                 else:
#                     random_subs = remaining.copy()
#                     if len(random_subs) < 3:
#                         fallback_pool = [k for k in LIST_KEYWORDS if k not in random_subs and k != sk]
#                         need = 3 - len(random_subs)
#                         if fallback_pool:
#                             random_subs.extend(random.sample(fallback_pool, min(need, len(fallback_pool))))

#                 chosen_subs = [sk] + random_subs[:3]

#                 date_tag = datetime.datetime.now().strftime("%m%d")
#                 suffix = f"{date_tag}{user_tag}|881比鸭"
#                 title = f"{app_fixed} - {url_fixed} - {'-'.join(chosen_subs)} - {suffix}"

#                 # Folder: stt-kw_chính
#                 folder_name = f"{str(stt_counter).zfill(stt_width)}-{sanitize_filename(sk)}"
#                 readme_filename = f"{folder_name}/README.md"
#                 content = generate_md_content(title, chosen_subs, domain_link, app_fixed, url_fixed)
#                 zf.writestr(readme_filename, content)

#                 # manifest chỉ lưu app - url - keywords
#                 manifest_entry_title = f"{app_fixed} - {url_fixed} - {'-'.join(chosen_subs)}"
#                 manifest_title_lines.append(manifest_entry_title)
...
#             manifest_content = "\n".join(f'"{line}",' for line in manifest_title_lines)
#             zf.writestr("danh_sach.txt", manifest_content)
